...
public class ClosedIntervalTest {
	...
	public void testLength() {
		testLength(new ClosedInterval(-20, -4), 16);
		testLength(new ClosedInterval(-20, 4), 24);
		testLength(new ClosedInterval(4, 20), 16);
	}

	public void testLength(ClosedInterval closedInterval, double length) {
		System.out.println("REQUIREMENTS: " + closedInterval + ".length ==" + length);
		System.out.println("SYSTEM: " + closedInterval + ".length ==" + (closedInterval.getLength()));
		assertThat(closedInterval.getLength(), is(length));
	}
